...
### Compressing Large Merged Results

Merged results can get large once every database’s rows for a task are
combined. Pass `compress = TRUE` to write the merged per-task result
files as gzip-compressed `.csv.gz` instead of plain `.csv`:
...
- [`readr::write_csv()`](https://readr.tidyverse.org/reference/write_delim.html)
  ...
  read `.csv.gz` files transparently on the way back in — no special
  handling needed to consume them, including in
  [`reviewExportSchema()`](https://ohdsi.github.io/Picard/reference/reviewExportSchema.md)
  and
  [`validateCohortResults()`](https://ohdsi.github.io/Picard/reference/validateCohortResults.md).
- Only the merged per-task result files are affected. Reference/QC files
  (`cohortManifestSnapshot.csv`, `databaseInfo.csv`,
  `schema_review.csv`, `qc_cohortValidation.csv`, `qc_processMeta.csv`)
  are always written as plain `.csv`, since those tend to stay small.
- Any script that reads a merged result file directly (e.g. a
  dissemination script) needs to account for the `.csv.gz` extension
  when `compress = TRUE` was used — either check for both extensions, or
  note in your own documentation which convention this study uses.
...
`disseminationEnv` is created for you when
[`sourceDisseminationScripts()`](https://ohdsi.github.io/Picard/reference/sourceDisseminationScripts.md)
runs, so it does not exist at the console while you are still writing
the script. Build the same object yourself with
[`createDisseminationEnv()`](https://ohdsi.github.io/Picard/reference/createDisseminationEnv.md)
so you can run the script line by line:
...
`pipelineVersion` is auto-detected from `config.yml` when omitted. The
returned object is identical to the one injected by
[`sourceDisseminationScripts()`](https://ohdsi.github.io/Picard/reference/sourceDisseminationScripts.md).
