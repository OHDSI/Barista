...
1.  Create study metadata (title, therapeutic area, study type,
    contributors)
2.  Define database configuration blocks (these carry all DBMS and
    schema info)
...
Bundle study metadata and database configuration blocks into
...
directory - `repoFolder`: Parent folder where the repository will be
created - `studyMeta`: StudyMeta object from Step 1 -
`dbConnectionBlocks`: List of `DbConfigBlock` objects from Step 2 (all
DBMS/schema info is contained in these blocks)
...
Before you can execute any pipelines, you need to configure database
credentials. These are stored securely in `~/.picard/secrets.yml` using
the keyring package.

**Interactive Setup (Recommended):**

Set up database server credentials interactively using
...
The function automatically opens
[`editSecrets()`](https://ohdsi.github.io/Picard/reference/editSecrets.md)
after setup so you can review the saved configuration.
...
This opens `~/.picard/secrets.yml` in your editor. You can add database
server blocks with either: - Plain text credentials (not recommended) -
`!expr keyring::key_get(...)` references (recommended - stores passwords
securely) - `!expr Sys.getenv(...)` references (for environment
variables)
...
If you plan to use ATLAS import builder scripts for cohorts or concept
sets, configure ATLAS credentials in `~/.picard/secrets.yml`.
...
Use these helpers to inspect and maintain credential configuration over
time.
