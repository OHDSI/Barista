...
5. Post-processing: `runPostProcessing()` merges raw results across databases into `dissemination/export/merge/`, with reference files and QC reports
6. Dissemination: numbered scripts in `dissemination/pretty/R/` format merged results via `sourceDisseminationScripts()`; the Study Hub (quarto website) renders narrative and results together
...
There is no `analysis/migrations/` folder in current Picard studies — post-processing and formatting now happen entirely under `dissemination/`, outside the versioned `analysis/` code. After `execStudyPipeline()` finishes, `runPostProcessing()` merges raw per-database results into `dissemination/export/merge/v{version}/`, and numbered scripts in `dissemination/pretty/R/` (created with `makeDisseminationScript()`, run with `sourceDisseminationScripts()`) handle the data wrangling and formatting that used to live in migration scripts — aggregating, standardizing, pivoting, deriving, and filtering results for dissemination. See [Post-Processing and Dissemination](post_processing.html) and the [Dissemination Folder](#dissemination-folder) section below for the full workflow.
...
This folder organizes results, evidence outputs, and documentation for sharing with stakeholders. Contains four main subdirectories: `documents/`, `quarto/`, `export/`, and `pretty/`.
...
Raw results merged across databases via `runPostProcessing()`, one subfolder per pipeline version:
...
`runPostProcessing(pipelineVersion, dbIds, ...)` reads raw results from `exec/results/` across all databases and tasks for a given version, merges each task's results via `importAndBind()`, adds a `databaseId` column, writes the reference/QC files above, and reviews schema with `reviewExportSchema()`. This folder holds durable, versioned raw exports — it is not touched by formatting scripts.

#### dissemination/pretty/

Where merged results get formatted for final release. Dissemination scripts live in `pretty/R/`, numbered like tasks, and their output is written alongside them by convention:
...
1. **Merge phase:** `runPostProcessing()` produces the versioned raw results in `dissemination/export/merge/v{version}/` described above.
2. **Author phase:** `makeDisseminationScript(name = "format_results")` creates a numbered template in `dissemination/pretty/R/`.
3. **Format phase:** `sourceDisseminationScripts()` sources every script in `dissemination/pretty/R/` in alphabetical order, injecting a `disseminationEnv` object (`pipelineVersion`, `databaseIds`, `outputPath`, `resultsPath`) so each script can read from `dissemination/export/merge/` and write formatted Excel/CSV/JSON output to `dissemination/pretty/` (or wherever `outputPath` points) — for publication tables, Study Hub data, or both. While authoring a script, call `createDisseminationEnv()` directly to build the same object at the console and run the script line by line before wiring it into the full pipeline.

See [Post-Processing and Dissemination](post_processing.html) for the full walkthrough, including QC reports and advanced usage of `importAndBind()`/`reviewExportSchema()`/`validateCohortResults()`.
